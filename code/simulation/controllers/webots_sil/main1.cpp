...
void run_square_test(webots::Robot *robot, Robot::Robot &robot_lib,
  ...
  std::vector<TrajectoryPhase> square = {
      ...
      {0.00f, 0.0f, stop_time}, {0.20f, 0.0f, 5.0f},
      {0.00f, 0.0f, stop_time}, {0.0f, 2.0f, turn_time}, // Поворот 90° (4)
  ...
  while (robot->step(time_step_ms) != -1 && phase_idx < square.size()) {
    ...
    if (current_phase_time >= phase.duration) {
      ...
      if (is_turn_phase) {
        ...
        printf("🔄 Поворот #%d: %.3f рад (%.1f°) | X: %.3f Y: %.3f\n",
               turn_number, angle_integral, degrees,
               gps ? gps->getValues()[0] : 0, gps ? gps->getValues()[1] : 0);
        angle_integral = 0.0;
        is_turn_phase = false;
      }
    ...
    if (!is_turn_phase && phase.angular_vel != 0.0f &&
        phase_idx % 3 == 2) { // Фазы поворотов: 2,6,10,14
      angle_integral = 0.0;
      is_turn_phase = true;
      turn_number++;
    ...
    if (is_turn_phase) {
      double imu_angular =
          robot_lib.m_imu.GetGyroZ(); // Твоя функция из UpdateSensors!
    ...
    Robot::ControlEffort effort =
        controller.GetAdjustedControlEffort(cmd, time_step_s);
    std::cout << "Усилие на левом моторе: " << effort.left_motor_voltage
              << std::endl;
    std::cout << "Усилие на правом моторе: " << effort.right_motor_voltage
              << std::endl;
    ...
    if (gps && tick++ % 15 == 0) {
      ...
      printf("[POS] X: %6.3f | Y: %6.3f | cmd: L=%.2f, A=%.2f", pos[0], pos[1],
...
void run_circle_test(webots::Robot *robot, Robot::Robot &robot_lib,
  ...
  float linear_vel =
      0.2f; // Линейная скорость (м/с) - можно увеличить, если едет рывками
...
RobotControl::MotionCommand getCosineTrajectory(float t) {
  ...
  float dy_dt =
      -A * k * Vx * sinf(k * Vx * t); // Первая производная (скорость по y)
  float d2y_dt2 = -A * k * k * Vx * Vx *
                  cosf(k * Vx * t); // Вторая производная (ускорение по y)
  ...
  // Формула: (dx*d2y - dy*d2x) / (v^2). Так как d2x = 0 (Vx постоянна), формула
  // упрощается:
...
int main(int argc, char **argv) {
  ...
  // Инициализируем новую FFModel (base_width=0.3, wheel_radius=0.05, kS=1.0,
  // kV=0.02)
  ...
  while (robot->step(time_step) != -1) {
    ...
    float angle_speed = robot_lib.m_last_state.current_angular_speed;
    if (gps) {
      double gps_speed = gps->getSpeed();
      float err = angular_velocity - angle_speed;
      printf("GPS_truth=%.3f m/s | Error=%.3f\n", gps_speed, err);
    }
  }
  return 0;
}
