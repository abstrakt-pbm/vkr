...
public:
  float GetRawLinearVelocity() const override { return m_raw_linear_velocity; }

  bool IsAlive() const override { return m_is_alive; }

  // Методы-хелперы для тестов
  void SetIsAlive(bool isAlive) { m_is_alive = isAlive; }

  void SetRawVelocity(float velocity) { m_raw_linear_velocity = velocity; }
...
TEST_F(EncoderTest, FilterConvergesOverTime) {
  ...
  // Прогоняем 30 итераций. При kAlpha=0.2 значение должно асимптотически
  // приблизиться к 50.0
  for (int i = 0; i < 30; i++) {
    encoder.UpdateState();
  }
...
TEST_F(EncoderTest, GetVelocityIsConstAndHasNoSideEffects) {
  ...
  // Многократный вызов не должен менять внутреннее состояние
  // (m_filtered_velocity_latest)
  EXPECT_FLOAT_EQ(encoder.GetCurrentVelocity(), 20.0f);
  EXPECT_FLOAT_EQ(encoder.GetCurrentVelocity(), 20.0f);
  EXPECT_FLOAT_EQ(encoder.GetCurrentVelocity(), 20.0f);
}
