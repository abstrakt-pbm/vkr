...
public:
  RobotController(Robot::Robot &robot, FFModel &ffmodel,
  ...
  Robot::ControlEffort
  GetAdjustedControlEffort(const MotionCommand &motion_command, float dt);
...
private:
  ...
  // Константы максимального ускорения (подбираются по формуле из предыдущего
  // ответа) 2.0 м/с^2 = разгон до 1 м/с за 0.5 секунды
