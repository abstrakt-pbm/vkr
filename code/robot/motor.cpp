...
namespace Robot {
Motor::Motor(HAL::IMotorHAL &motor_hal, float ramp_coeff, float max_voltage,
             float voltage_to_start)
    : m_motor_hal(motor_hal), m_ramp_coeff(ramp_coeff),
      m_max_voltage(max_voltage), m_min_voltage_to_start(voltage_to_start),
...
void Motor::SetVoltage(float target_voltage, float dt) {
  ...
  /*
  if (std::abs(target_voltage) >  1e-6f &&  std::abs(target_voltage) <
  m_min_voltage_to_start) { SetRawVoltage(m_min_voltage_to_start);
  }
...
float Motor::GetCurrentVoltage() { return m_motor_hal.GetCurrentRawVoltage(); }

bool Motor::IsAlive() { return m_motor_hal.IsAlive(); }

float Motor::GetMaxVoltage() const { return m_max_voltage; }
