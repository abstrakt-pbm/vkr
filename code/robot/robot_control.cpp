...
RobotController::RobotController(Robot::Robot &robot,
	...
	m_max_linear_accel(200.0f),
	m_max_angular_accel(200.0f),
...
MotionCommand RobotController::VelocityRamp(const MotionCommand& target_cmd, float dt) {
	...
	LOG_INFO("RobotController: Smooth CMD linear velocity: %.3f m/s", smooth_cmd.linear_velocity);
	LOG_INFO("RobotController: Smooth CMD angular velocity: %.3f r/s", smooth_cmd.angular_velocity);
    return smooth_cmd;
}
} // namespace RobotControl
