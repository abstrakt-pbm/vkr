...
FFModel::FFModel(float base_width, float wheel_radius, float kS, float kV, float max_voltage)
...
float FFModel::CalculateMotorVoltage(float target_omega) const {
    ...
    float voltage = (m_kS * std::copysign(1.0f, target_omega)) + (m_kV * target_omega);
...
Robot::ControlEffort FFModel::GetControlEffort(const MotionCommand& cmd) const {
    ...
    float v_left  = cmd.linear_velocity - (cmd.angular_velocity * m_base_width / 2.0f);
    float v_right = cmd.linear_velocity + (cmd.angular_velocity * m_base_width / 2.0f);
